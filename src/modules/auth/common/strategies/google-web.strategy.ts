...
import { IGoogleWebUserData, IThirdPartyAuthWebState } from "src/modules/auth/common/interfaces";
...
export class GoogleWebStrategy extends PassportStrategy(Strategy, AuthStrategies.GOOGLE_WEB_STRATEGY) {
  ...
  async validate(
    ...
    const state: IThirdPartyAuthWebState = JSON.parse(req.query.state as string) as IThirdPartyAuthWebState;
    // eslint-disable-next-line @typescript-eslint/no-unsafe-assignment
    const { name, emails, photos } = profile;
    const user: IGoogleWebUserData = {
