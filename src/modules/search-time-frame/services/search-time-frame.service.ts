...
import {
  ...
  NUMBER_OF_HOURS_IN_HALF_DAY,
  NUMBER_OF_HOURS_IN_HOUR,
  NUMBER_OF_HOURS_IN_QUARTER_DAY,
  NUMBER_OF_HOURS_IN_TEN_HOURS,
  NUMBER_OF_HOURS_IN_THREE_HOURS,
  NUMBER_OF_HOURS_IN_THREE_QUARTERS_OF_DAY,
...
import { EAppointmentCombinationType, ETimeFrameCategory } from "src/modules/search-time-frame/common/enum";
import { ITimeFrame, ITimeFrames } from "src/modules/search-time-frame/common/interface";
  ...
  private determineTimeCategory(
    ...
    if (appointmentType === EAppointmentCombinationType.VIDEO_AUDIO) {
      if (daysUntilAppointment > NUMBER_OF_DAYS_IN_MONTH && daysUntilAppointment <= NUMBER_OF_DAYS_IN_HALF_YEAR) {
        ...
        hoursUntilAppointment > NUMBER_OF_HOURS_IN_QUARTER_DAY &&
        daysUntilAppointment <= NUMBER_OF_DAYS_IN_THREE_DAYS
      ) {
      ...
      if (daysUntilAppointment > NUMBER_OF_DAYS_IN_MONTH && daysUntilAppointment <= NUMBER_OF_DAYS_IN_HALF_YEAR) {
        ...
        hoursUntilAppointment > NUMBER_OF_HOURS_IN_THREE_QUARTERS_OF_DAY &&
        daysUntilAppointment <= NUMBER_OF_DAYS_IN_THREE_DAYS
      ) {
    ...
    if (appointmentType === EAppointmentCombinationType.VIDEO_AUDIO) {
      switch (timeCategory) {
        ...
        case ETimeFrameCategory.ONE_MONTH_TO_THREE_DAYS:
          intervalMinutes = NUMBER_OF_HOURS_IN_QUARTER_DAY * NUMBER_OF_MINUTES_IN_HOUR;
    ...
    if (appointmentType === EAppointmentCombinationType.VIDEO_AUDIO) {
      ...
      switch (timeCategory) {
        ...
        case ETimeFrameCategory.ONE_MONTH_TO_THREE_DAYS:
          ...
          break;
        case ETimeFrameCategory.THREE_DAYS_TO_EIGHTEEN_HOURS:
          endTime = new Date(currentTime.getTime() + NUMBER_OF_HOURS_IN_HALF_DAY * NUMBER_OF_MILLISECONDS_IN_HOUR);
          break;
        case ETimeFrameCategory.EIGHTEEN_HOURS_TO_THREE_HOURS:
