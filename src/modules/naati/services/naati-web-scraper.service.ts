...
export class NaatiWebScraperService {
  ...
  public async updateNonceToken(): Promise<MessageOutput> {
    try {
  ...
  public async launchBackgroundFullUpdate(interpreterType: EExtNaatiInterpreterType): Promise<MessageOutput> {
    ...
    this.fullUpdateDatabase(interpreterType)
      .then(() => {
        const report = `
            Background Naati update completed successfully.
            Total Languages Processed: ${this.totalLanguagesProcessed}
            Total Profiles Processed: ${this.totalProfilesProcessed}
            Total Errors: ${this.totalErrors}
          `;
        this.lokiLogger.log(`Background Naati update completed successfully`);
        void this.sendEmailsToAdminsInBackground(report);
      })
      .catch((error: Error) => {
        const report = `
            Error in Naati background update, error: ${error}.
            Total Languages Processed: ${this.totalLanguagesProcessed}
            Total Profiles Processed: ${this.totalProfilesProcessed}
            Total Errors: ${this.totalErrors}
          `;
  ...
  private async getAllNaatiInterpreterLanguages(
    ...
    params.append("require", interpreterType);
    params.append("action", "naati_od_get_languages");
    params.append("nonce", this.DIRECTORY_NONCE_TOKEN);
    params.append("conference", "false");
  ...
  private async getNaatiInterpretersByLanguage(
    ...
    try {
      const params = new URLSearchParams();
      params.append("nonce", this.DIRECTORY_NONCE_TOKEN);
      params.append("_wp_http_referer", "/online-directory/");
      params.append("action", "naati_od_get_search_results");
      language.SkillIds.forEach((id) => params.append("forLanguages[]", id.toString()));
      params.append("randomSeed", randomSeed);
      params.append("require", interpreterType);
      params.append("page", page.toString());
      params.append("certificationType", "0");
      params.append("country", "0");
  ...
  private async sendEmailsToAdminsInBackground(massage: string): Promise<void> {
    const superAdmins = await this.helperService.getSuperAdmin();

    for (const superAdmin of superAdmins) {
      await this.emailsService.sendNaatiWebScraperNotifyToAdmin(superAdmin.email, massage);
    }
  }
}
