...
import { INaatiApiResponse, INaatiCertifiedLanguagesList } from "src/modules/naati/common/interface";
...
export class NaatiController {
  ...
  async getInternalNaatiInfo(@CurrentUser() user: ITokenUserData): Promise<INaatiCertifiedLanguagesList> {
    return this.naatiService.findCurrentUserInInternalDatabase(user);
  }

  @Post("/cpn-info-saving")
  @UseGuards(JwtRequiredInfoOrActivationOrFullAccessGuard, RolesGuard)
  async saveCpnNaatiInfo(@CurrentUser() user: ITokenUserData, @Query() dto: NaatiCpnQueryDto): Promise<MessageOutput> {
  ...
  async getCpnNaatiInfo(@Query() dto: NaatiCpnQueryDto): Promise<INaatiApiResponse> {
