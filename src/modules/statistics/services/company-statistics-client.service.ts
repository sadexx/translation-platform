...
export class CompanyStatisticsClientService {
  ...
  public async getCancelledAppointmentsByCompany(
    ...
    for (const roleName of dto.roleNames) {
      let userRoleName: EUserRoleName | undefined = undefined;
  ...
  public async getHomepageBaseAppointmentInfoByCompanyId(
    ...
    const appointmentDurationQuery =
      ...
      `AND appointments.status IN ('${EAppointmentStatus.COMPLETED}', '${EAppointmentStatus.COMPLETED_ACTION_REQUIRED}') `;
  ...
  public async getSpentCostByCompany(dto: GetSpentCostByCompany, user: ITokenUserData): Promise<IChartRoundData> {
    const company = await this.helperService.getCompanyByRole(user, {}, dto.companyId);
