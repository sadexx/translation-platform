...
export class CompaniesController {
  ...
  @Get("my-company")
  public async getCompanyByUser(@CurrentUser() user: ITokenUserData): Promise<IGetMyCompany | null> {
  ...
  @UseGuards(JwtRequiredInfoOrActivationOrFullAccessGuard, RolesGuard)
  @HttpCode(HttpStatus.NO_CONTENT)
  @Delete("delete-company-request")
  public async deleteCompanyRequest(
    @Query() dto: DeleteCompanyDto,
    @CurrentUser() user: ITokenUserData,
  ): Promise<void> {
    return await this.companiesService.deleteCompanyRequest(dto, user);
  }

  @UseGuards(JwtRestorationGuard, RolesGuard)
  @Post("restore-company")
  public async restoreCompany(@CurrentUser() user: ITokenUserData): Promise<void> {
    return await this.companiesService.restoreCompany(user);
  }
