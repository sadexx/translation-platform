...
import { ICreateContractInterface, ISendContractInterface } from "src/modules/docusign/common/interfaces";
...
export class DocusignCorporateController {
  ...
  async createCorporateContract(
    @Body() { companyId }: CorporateContractDto,
    @CurrentUser() user: ITokenUserData,
  ): Promise<ICreateContractInterface> {
    return this.docusignCorporateService.createCorporateContract(companyId, user);
  }

  @UseGuards(JwtRequiredInfoOrActivationOrFullAccessGuard, RolesGuard)
  @Post("send-corporate-contract")
  async sendCorporateContract(@Body() { id }: SendContractDto): Promise<ISendContractInterface> {
  ...
  async getContractEditLink(@Query() { id }: SendContractDto): Promise<ICreateContractInterface> {
