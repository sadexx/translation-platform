...
export class DocusignCorporateService {
  ...
  public async createAndSendCorporateContract(
    companyId: string,
    user: ITokenUserData,
  ): Promise<ISendContractInterface> {
  ...
  public async createCorporateContract(companyId: string, user: ITokenUserData): Promise<ICreateContractInterface> {
    ...
    const templateId = this.docusignService.getTemplateId(
      company.superAdmin.userRoles[0].role.name,
      companyCountry,
      signerCount,
    );
  ...
  public async fillAndSendCorporateContract(contractId: string): Promise<ISendContractInterface> {
  ...
  public async getEditLink(contractId: string): Promise<ICreateContractInterface> {
    const contract = await this.docusignContractRepository.findOne({
      where: { id: contractId },
    });
