...
import { ADMIN_ROLES, CLIENT_ROLES } from "src/common/constants";
...
export class DraftAppointmentService {
  ...
  public async getAllDraftAppointmentsForAdmin(
    ...
    this.draftAppointmentQueryOptionsService.getAllDraftAppointmentsForAdminOptions(queryBuilder, dto);

    const [draftAppointments, count] = await queryBuilder.getManyAndCount();
    const instances = plainToInstance(DraftAppointmentOutput, draftAppointments, {
      groups: ADMIN_ROLES,
    });
  ...
  public async createFullDraftAppointment(dto: CreateDraftAppointmentsDto): Promise<MessageOutput> {
  ...
  public async deleteDraftAppointment(id: string, user: ITokenUserData): Promise<MessageOutput> {
