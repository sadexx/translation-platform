...
export class ChannelGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect {
  ...
  private async syncActiveChannelAndNotifyMembers(channel: Channel, userRoleId: string): Promise<void> {
    for (const member of channel.memberships) {
      if (member.externalUserId === userRoleId) {
        ...
        if (channel.channelArn && activeChannelArn !== channel.channelArn) {
          ...
          this.messagingManagementService
            .resetUnreadCounterByUser(channel.channelArn, member.externalUserId)
            .catch((error: Error) => {
              this.lokiLogger.error(
                `Failed to reset unread counter for userRoleId: ${member.externalUserId}`,
                error.stack,
              );
            });
        }
      ...
      const activeChannelArn = await this.activeChannelStorageService.getActiveChannel(member.externalUserId);

      if (activeChannelArn !== channel.channelArn) {
        this.messagingManagementService.incrementUnreadCounter(channel, userRoleId).catch((error: Error) => {
          this.lokiLogger.error(`Failed to increment unread counter error: ${error.message}, ${error.stack}`);
        });
        this.notificationService
          .sendChannelMessageNotification(member.externalUserId, channel.platformId, {
            channelId: channel.id,
            channelType: channel.type,
          })
          .catch((error: Error) => {
            this.lokiLogger.error(
              `Failed to send new message notification for userRoleId: ${member.externalUserId}`,
              error.stack,
            );
          });
      }
