...
import { IAllTypeAppointmentOrdersForWebsocket } from "src/modules/appointment-orders/common/interface";
...
export class OrderGateway implements OnGatewayInit, OnGatewayConnection, OnGatewayDisconnect, OnModuleDestroy {
  ...
  async handleMessage(@ConnectedSocket() client: Socket, @MessageBody() message: OrderEventDto): Promise<void> {
    try {
  ...
  async broadcastNewOrders(appointmentOrdersOutput: IAllTypeAppointmentOrdersForWebsocket): Promise<void> {
      ...
      for (const newOrder of appointmentOrders) {
        for (const { user, socketId } of allConnections) {
          ...
          if (
            ...
            const serializedIndividualOrder = plainToInstance(AppointmentOrderOutput, newOrder);
            try {
      ...
      for (const newGroupOrder of appointmentOrdersGroups) {
        for (const { user, socketId } of allConnections) {
          ...
          if (
            ...
            const serializedGroupOrder = plainToInstance(AppointmentOrderGroupOutput, newGroupOrder);
            try {
