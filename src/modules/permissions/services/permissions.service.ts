...
import { appointmentsOrder } from "src/modules/appointment-orders/common/permissions-seeds";
import { appointments } from "src/modules/appointments/common/permissions-seeds";
...
import { interpreterProfile } from "src/modules/interpreter-profile/common/permissions-seeds";
...
import { IGetPermissions, IGetPermissionsModules } from "src/modules/permissions/common/interfaces";
import { multiWayParticipants } from "src/modules/multi-way-participant/permissions-seeds/seeds";
import { addresses } from "src/modules/addresses/common/permissions-seeds";
import { interpreterQuestionnaire } from "src/modules/interpreter-questionnaire/common/permissions-seeds";
...
import { payments } from "src/modules/payments/common/permissions-seeds";
import { rates } from "src/modules/rates/common/permissions-seeds";
import { interpreterBadge } from "src/modules/interpreter-badge/common/permissions-seeds";
...
export class PermissionsService {
  ...
  public async getPermissions(userRoleName: EUserRoleName): Promise<IGetPermissions> {
    let permissions = await this.redisService.getJson<IGetPermissions>(userRoleName);
      ...
      if (layer.route) {
        ...
        if (path && method) {
          ...
          for (const role of roles) {
            let methodIsAllowedForRole: boolean = endpointSeed.roles[role.name]?.isAllowed;
            let methodIsEditableForRole: boolean = Boolean(endpointSeed.roles[role.name]?.isEditable);

            if (!endpointSeed.roles[role.name]) {
              methodIsAllowedForRole = false;
              methodIsEditableForRole = !endpointSeed.isNotEditableForOtherRoles;
            }

            if (endpointSeed.roles[role.name] && endpointSeed.roles[role.name].isEditable === undefined) {
              methodIsEditableForRole = true;
            }
  ...
  public async getPermissionsByRole(userRoleName: EUserRoleName): Promise<IGetPermissions> {
    const methods = await this.methodRepository.find({
