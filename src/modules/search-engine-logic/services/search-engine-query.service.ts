...
import { ELanguageLevel, ESignLanguages } from "src/modules/interpreter-profile/common/enum";
...
@Injectable()
export class SearchEngineQueryService {
  ...
  public async applyWorkingHoursForAllCompanyCondition(
    ...
    query.select(["userRole.id"]);
    query.innerJoin("interpreter.userRole", "userRole").innerJoin("userRole.role", "role");
    query.andWhere(
      `EXISTS (
      SELECT 1
      FROM language_pairs
  ...
  public async applyWorkingHoursForAllCompanyOnDemandCondition(
    ...
    query.select(["userRole.id"]);
    query.innerJoin("interpreter.userRole", "userRole").innerJoin("userRole.role", "role");
    query.andWhere("interpreter.endOfWorkDay > :currentDate", { currentDate: new Date() });
    query.andWhere(
      `EXISTS (
      SELECT 1
      FROM language_pairs
