...
import {
  ...
  QuestionnaireOutput,
} from "src/modules/interpreter-questionnaire/common/outputs";
import {
...
export class InterpreterQuestionnaireController {
  ...
  create(@Body() dto: CreateInterpreterQuestionnaireDto, @CurrentUser() user: ITokenUserData): Promise<MessageOutput> {
  ...
  @Patch("questionnaire")
  @SerializeOptions({ type: QuestionnaireOutput })
  update(@Body() dto: UpdateInterpreterQuestionnaireDto, @CurrentUser() user: ITokenUserData): Promise<MessageOutput> {
